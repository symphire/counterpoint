...
use crate::domain::{AuthError, StorageTx, UserId};
use crate::infra::{UserRecord, UserRepo};
...
"#,
    ...
    async fn get_id_by_username_in_tx<'t>(&self, tx: &mut dyn StorageTx<'t>, username: &str) -> Result<UserId, AuthError> {
        let tx = downcast(tx);
    ...
    async fn username_exists(&self, username: &str) -> Result<bool, AuthError> {
        let count: i64 =
            sqlx::query_scalar(r#"SELECT COUNT(*) FROM user WHERE username = ?"#)
                .bind(username)
                .fetch_one(&self.pool)
                .await
                .map_err(|e| AuthError::Store(e.to_string()))?;
