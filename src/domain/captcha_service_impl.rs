use crate::domain::{CaptchaError, CaptchaResult, CaptchaService, ValidationInput};
use crate::infra::{CaptchaStore, CaptchaStoreError};
...
impl RealCaptchaService {
    ...
    fn hmac_hex(&self, code: &str) -> Result<String> {
...
impl CaptchaService for RealCaptchaService {
    async fn generate(&self) -> Result<CaptchaResult, CaptchaError> {
        ...
        let id = domain::CaptchaId(uuid::Uuid::new_v4());
        ...
        let clean = with_prefix.split_once(',').map(|(_, d)| d).unwrap_or(with_prefix.as_str());
        Ok(CaptchaResult {
            id,
